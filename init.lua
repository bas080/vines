-- luacheck: globals vines
-- luacheck: read_globals luanti_utils default
--
-- TODO: Instead of doing air check it might be better to create
-- helpers to check if something is growable into or onto.
...
local function on_dig(vine_name_end)
	return function(pos, node, player)
		...
		if wielded_item then
			...
			local dig_params =
				core.get_dig_params(node_def.groups, wielded_item:get_tool_capabilities(), wielded_item:get_wear())
...
vines.register_vine = function(name, defs, def)
	...
	local on_grow = function(pos)
		...
		if node.param2 > 3 then -- is growing downward
			...
			local next_bottom_node = core.get_node(next_bottom_pos)
			...
			if next_bottom_node.name == "air" then
				return grow(next_bottom_pos, flat_to_down[next_param2])
			end

			if is_buildable_to(next_bottom_pos) then
				return nil
			end
...
if enable_roots ~= false then
	vines.register_vine("root", { description = S("Roots"), average_length = 9 }, {
		place_on = {
...
if enable_standard ~= false then
	vines.register_vine("vine", { description = S("Vines"), average_length = 8 }, {
		place_on = {
...
if enable_side ~= false then
	vines.register_vine("side", { description = S("Vines"), average_length = 6 }, {
		place_on = {
			"default:jungleleaves",
			"moretrees:jungletree_leaves_red",
			"moretrees:jungletree_leaves_yellow",
...
if enable_jungle ~= false then
	vines.register_vine("jungle", { description = S("Jungle Vines"), average_length = 7 }, {
		place_on = {
			"default:jungletree",
...
if enable_willow ~= false then
	vines.register_vine("willow", { description = S("Willow Vines"), average_length = 9 }, {
		flags = "all_floors",
